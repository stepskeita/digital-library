...
import { GET_BOOKS_ERROR } from "../reducers/types/bookTypes";

const KeywordSearch = () => {
  const { slug } = useParams();
  const { loading, books, error, page, total } = useSelector(
    (state) => state.getBooks
  );
  const history = useHistory();
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getBooks(`${backendApiUrl}/book?keyword=${slug}`));
    scrollToTop();
  }, [dispatch, slug]);
  return (
    <Container>
      <CustomTitle title={`Keyword: ${slug}`} />
      <div className="flex items-center text-sm mb-5">
        ...
        <>
          ...
          <p className="truncate">
            Keyword: <span className="font-bold">{slug}</span>
          </p>
        </>
      </div>
      ...
      {loading ? (
        ...
        <ErrorAlert
          text={error}
          handleClose={() => dispatch({ type: GET_BOOKS_ERROR })}
        />
      ) : (
        books && (
          <>
